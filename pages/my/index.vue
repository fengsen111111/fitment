		...
		<view class="userBg">
			<view class="flex justify-between items-center p30">
				...
				<view class="flex">
					...
					<uni-icons @click="handUrl('/pages/my/components/system/index')" type="gear" class="ml40" size="30" color="#FFFFFF"></uni-icons>
				</view>
			</view>
			<view class="px30">
				...
				<view class="mt20 text20 text-center col-black bg-white p40 flex justify-between radius10">
					<view @click="handUrl('/pages/my/components/orderList/index?status='+item.text)" class="" v-for="item in orderList" :key="item.id">
				...
				<view class="mt30 text24 text-center col-black bg-white p40 grid grid-cols-3 radius10">
					<view :class="item.id<4?'mb60': item.id>6?'':'mb60'" v-for="item in iconList" :key="item.id" @click="handUrl(item.url)">
